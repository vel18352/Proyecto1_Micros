...
isr: ;Sub Rutinas de interrupcion
    btfsc ((INTCON) and 07Fh), 0 ;Se chequea Interrupcion del puerto B
    call Cambiar_Limites ;Se llama funcion de incrementar o decrementar Puerto B
    btfsc ((PIR1) and 07Fh), 1 ;Se chequea la bandera de interrupcion TMR2
    call Mostrar_Display ;Se llama a la funcion de display
    btfsc ((INTCON) and 07Fh), 2 ;Se chequea Interrupcion de Timer0
...
int_iocbHz:
    ...
    bcf ((INTCON) and 07Fh), 0 ;Se limpia bandera de interrupcion
    return
...
;------------ LIMPIAR PUERTO -------
    ...
    bcf STATUS, 1 ;Limpia ((STATUS) and 07Fh), 2
...
config_iocb:
    ...
    bcf ((INTCON) and 07Fh), 0 ;Se limpia la bandera de interrupcion del Puerto B
    return
...
config_reloj:
    ...
    bsf ((OSCCON) and 07Fh), 4 ;Frecuencia de 8 MHz
...
config_int_enable:
    ...
    bsf ((INTCON) and 07Fh), 3 ;Se habilita la interrupcion de Puerto B
    bcf ((INTCON) and 07Fh), 0 ;Se limpia la bandera de interrupcion del Puerto B
    bsf ((INTCON) and 07Fh), 5 ;Se habilita interrupcion TMR0
    bcf ((INTCON) and 07Fh), 2 ;Se limpia bandera de TMR0
    bsf ((PIE1) and 07Fh), 1
    bcf ((PIR1) and 07Fh), 1
    return
...
CONFIG_TMR0:
    ...
    bcf ((OPTION_REG) and 07Fh), 0 ;010 prescaler 1 : 8
    banksel TMR0 ;Se cambia de banco
    movlw 128
    movwf TMR0 ;Se carga Pre carga a TMR0
    bcf ((INTCON) and 07Fh), 2 ;Se limpia bandera de interrupción
    return
;----------- REINICIAR EL TIMER0 -----------
reinicio_TMR0:
    banksel TMR0
    ...
    bcf ((INTCON) and 07Fh), 2 ;Se limpia bandera de interrupcion
    return
;--------------- TIMER 2----------------------
Config_TMR2:
    banksel T2CON ;Selecciona banco TMR2
    bsf ((T2CON) and 07Fh), 2 ;Encender TMR2
    bcf ((T2CON) and 07Fh), 1
    bcf ((T2CON) and 07Fh), 0 ;Prescaler a 1:1
    bcf ((T2CON) and 07Fh), 6
    bcf ((T2CON) and 07Fh), 5
    bcf ((T2CON) and 07Fh), 4
    bcf ((T2CON) and 07Fh), 3 ;Postscaler 1:1
    movlw 180
    movwf PR2 ;Mover 180 a PR2
    bcf ((PIR1) and 07Fh), 1 ;Limpiar bandera de interrupcion
    return
Reset_TMR2:
    movlw 180
    ...
    bcf ((PIR1) and 07Fh), 1 ;Limpiar Interrupcion TMR2
    return
toggle_led:
